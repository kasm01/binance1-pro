...
from core.redis_price_cache import read_kline_cache, write_kline_cache, RedisPriceCache, RedisPriceCache
        ...
        try:
            if raw_df is not None and len(raw_df) > 0:
                ...
                if close_val and close_val > 0:
                    pc = RedisPriceCache(
                        key_prefix=f"{os.getenv('REDIS_PREFIX','binance1_pro')}:pricecache",
                        ttl_sec=int(os.getenv("REDIS_PRICECACHE_TTL_SEC", "60")),
                    )
        ...
        try:
            if (not cache_hit) and raw_df is not None and len(raw_df) >= 30:
                write_kline_cache(
                    symbol=symbol,
                    interval=interval,
                    rows=raw_df.tail(100).to_dict("records"),
                    ttl_sec=90,
                )
        except Exception:
            pass
...
async def async_main() -> None:
    ...
    redis_price_cache = RedisPriceCache(
        key_prefix=f"{os.getenv('REDIS_PREFIX', 'binance1_long')}:pricecache",
        ttl_sec=int(os.getenv("REDIS_PRICECACHE_TTL_SEC", "120")),
    )
